...
export class TelegramService implements FacegramService {
  ...
  async initialize () {
    ...
    const code = await new Promise(result => rl.question('Enter authorization code sent to your telegram account: ', result))
    ...
    try {
      ...
      const password = await new Promise(result => rl.question('Enter your telegram account\'s password: ', result))
